...
(function(root){
 'use strict';
 const defaults={designer:'',layout:'',reachCount:'',shape:'',lining:'',condition:'',qDesign:'',depth:'',base:'',sideSlope:'',gradient:'',manning:''};
 const linings={
 ...
 function calculate(input){
  ...
  if(!['single','continuous'].includes(input.layout))errors.push('Drain arrangement');
 ...
 function restoreState(state){if(!state||typeof state!=='object')return false;draft={...defaults};for(const key of Object.keys(defaults))if(Object.prototype.hasOwnProperty.call(state,key))draft[key]=state[key];if(draft.layout==='single')draft.reachCount=1;if(draft.layout==='continuous')draft.reachCount=Math.min(8,Math.max(2,Number(draft.reachCount)||2));return true}
 ...
 const pointName=i=>String.fromCharCode(65+i);
 function routeCount(){return draft.layout==='single'?1:draft.layout==='continuous'?Math.min(8,Math.max(2,Number(draft.reachCount)||2)):1}
 function routeText(){return Array.from({length:routeCount()},(_,i)=>`${pointName(i)}–${pointName(i+1)}`).join(' · ')}
 function reachList(){
  if(!draft.layout)return '<p class="dc-route-empty">Select Single or Continuous drain to define the route.</p>';
  const reaches=Array.from({length:routeCount()},(_,i)=>`<div class="dc-reach"><span>Reach ${String(i+1).padStart(2,'0')}</span><b>${pointName(i)} <i>→</i> ${pointName(i+1)}</b></div>`).join('');
  return `<div class="dc-reach-list">${reaches}</div>${draft.layout==='continuous'?`<div class="dc-route-actions"><button type="button" id="dc-remove-reach" ${routeCount()<=2?'disabled':''}>Remove last reach</button><button type="button" id="dc-add-reach" ${routeCount()>=8?'disabled':''}>Add next reach</button></div>`:''}`;
 }
 function planDiagram(id='screen'){
  const count=routeCount(),points=count+1,coords=Array.from({length:points},(_,i)=>({x:55+i*(540/Math.max(1,count)),y:105+(i%2?20:-20)})),path=coords.map((p,i)=>`${i?'L':'M'}${p.x} ${p.y}`).join(' '),ready=Boolean(draft.layout);
  const reaches=ready?coords.slice(0,-1).map((p,i)=>{const next=coords[i+1],mx=(p.x+next.x)/2,my=(p.y+next.y)/2;return `<text x="${mx}" y="${my-16}" text-anchor="middle">${pointName(i)}–${pointName(i+1)}</text>`}).join(''):'';
  const nodes=ready?coords.map((p,i)=>`<circle cx="${p.x}" cy="${p.y}" r="10"/><text class="dc-point-label" x="${p.x}" y="${p.y+5}" text-anchor="middle">${pointName(i)}</text>`).join(''):`<text class="dc-plan-empty" x="325" y="110" text-anchor="middle">Select a drain arrangement to show points</text>`;
  return `<svg class="dc-plan" viewBox="0 0 650 200" role="img" aria-label="Drain plan view ${ready?routeText():'awaiting arrangement'}"><defs><marker id="dc-plan-arrow-${id}" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto"><path d="M0 0L10 5L0 10Z" fill="#2a6555"/></marker></defs>${ready?`<path class="dc-plan-line" d="${path}" marker-end="url(#dc-plan-arrow-${id})"/>`:''}<g class="dc-plan-labels">${reaches}</g><g class="dc-plan-nodes">${nodes}</g>${ready?'<text class="dc-flow-label" x="325" y="178" text-anchor="middle">Flow direction · schematic plan</text>':''}</svg>`;
 }
 ...
 function update(){
  ...
  document.querySelector('#dc-print').disabled=!r.valid;document.querySelector('#dc-results').innerHTML=resultHtml(r);document.querySelector('#dc-route').innerHTML=reachList();document.querySelector('#dc-plan').innerHTML=planDiagram();document.querySelector('#dc-section-title').textContent=`Cross-Section${draft.shape?` · ${draft.shape[0].toUpperCase()+draft.shape.slice(1)}`:''}`;document.querySelector('#dc-section').innerHTML=sectionDiagram(r);document.querySelector('#dc-report').innerHTML=report(r);
  const add=document.querySelector('#dc-add-reach'),remove=document.querySelector('#dc-remove-reach');if(add)add.addEventListener('click',()=>{draft.reachCount=Math.min(8,routeCount()+1);update()});if(remove)remove.addEventListener('click',()=>{draft.reachCount=Math.max(2,routeCount()-1);update()});
 ...
 function mount(container){
  ...
  form.addEventListener('input',e=>{if(!e.target.name)return;draft[e.target.name]=e.target.value;if(e.target.name==='layout')draft.reachCount=e.target.value==='single'?1:2;if(e.target.name==='shape')syncConditional();if(e.target.name==='lining'&&linings[e.target.value]){draft.manning=linings[e.target.value].n;form.elements.manning.value=draft.manning}update()});
