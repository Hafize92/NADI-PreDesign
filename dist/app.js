...
const moduleStatuses={'0/0':'incomplete','3/0':'incomplete'};
...
function saveProject(){
 ...
 const payload={format:'NADI-PreDesign',schemaVersion:1,appVersion:APP_VERSION,savedAt:new Date().toISOString(),project:{name,siteArea:siteArea.value},modules:{earthDrain:EarthDrain.getState(),drainageCheck:DrainageCheck.getState()},statuses:{...moduleStatuses}};
...
async function restoreProject(file){
 try{
  ...
  projectName.value=data.project.name||'';siteArea.value=data.project.siteArea??'';EarthDrain.restoreState(data.modules.earthDrain||{});DrainageCheck.restoreState(data.modules.drainageCheck||{});
