...
const moduleStatuses={'0/0':'incomplete'};
...
function render(){
 ...
 if(match){
  ...
  detail.innerHTML=`<a class="back" href="${sub?'#module/'+i:'#dashboard'}">← ${sub?'Back to '+m.name:'Back to dashboard'}</a><div class="detail-panel"><span class="status">Awaiting details</span><h2>${sub||m.name}</h2>${!sub&&m.subs.length?'<p>Components identified for this module.</p>'+m.subs.map((s,j)=>`<a class="submodule" href="#module/${i}/${j}">${s}<span class="status" data-submodule-status="${i}/${j}">${statusLabel(moduleStatuses[`${i}/${j}`])}</span></a>`).join(''):''}<div class="empty"><h3>Design details to follow</h3><p>${sub?'Parameters, criteria, and calculation methods for '+sub+' will be added when the details are available.':'The design scope, parameters, and criteria will be completed when further information is available.'}</p><p>No calculations or design results are available at this stage.</p></div></div>`;
...
function saveProject(){
 ...
 const payload={format:'NADI-PreDesign',schemaVersion:1,appVersion:APP_VERSION,savedAt:new Date().toISOString(),project:{name,siteArea:siteArea.value},modules:{earthDrain:EarthDrain.getState()},statuses:{...moduleStatuses}};
...
async function restoreProject(file){
 try{
  ...
  projectName.value=data.project.name||'';siteArea.value=data.project.siteArea??'';EarthDrain.restoreState(data.modules.earthDrain||{});
